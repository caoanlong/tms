<template>
	<div class="main-content" style="padding-left:200px">
		<el-form label-width="126px" size="mini" :model="superCargo" :rules="rules" ref="ruleForm" >
			...
			<el-card class="box-card">
				<span style="position:relative;top:20px">从事运输岗位</span>
				<el-form-item prop="supercargoType">
					<el-checkbox-group v-model="supercargoType" @change="supercargoTypeChange">
						<el-checkbox label="Driver" :disabled="supercargoType.includes('Driver')">驾驶员</el-checkbox>
			...
			<el-card class="box-card" v-show="supercargoType.includes('Driver')">
				...
				<el-row :gutter="20" style="margin-bottom:20px">
					<el-col :span="12">
						<div class="section-block target6">
	...
	methods: {
		...
		getInfo() {
			...
			SuperCargo.findById({ comSupercargoID }).then(res => {
				...
				this.driverLicenseTime = [res.driverLicenseBeginTime, res.driverLicenseEndTime]
				this.qualificationDate = [res.qualificationBeginDate, res.qualificationExpirationDate]
				this.escortLicenseDate = [res.escortLicenseBeginDate, res.escortLicenseExpireDate]
				this.superCargo.qualificationIsNewest = res.qualificationIsNewest == 'Y' ? true : false
